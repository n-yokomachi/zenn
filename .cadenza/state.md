...
## Phase 4: Analysis Execution (⏸ 一部完了・中断中)

### 完了した検証（先行バッチ：実装を伴わない部分）
- SQ2 比較表、SQ3 二層ループ図（Mermaid）、affectus CLI 実出力の採取
- 成果物：`./.cadenza/materials/concept-batch.md`
- 記事末尾の参考文献を5件に確定

### 未実施（再開時に実行）
- SQ4：affectus の TONaRi 統合、3シナリオ実験、AWS Comprehend 感情分析、チャート作成
- ユーザー指示により検証を一旦停止し、ベース記事を先に作成した。

---

## Phase 5: Output Crafting (✅ Done 2026-05-18 / ベース版)

### Generated output
- `./.cadenza/output.md`
- タイトル：動的な感情制御はAIエージェントとの会話を深くするか

### 注記
- ユーザー指示により Phase 4（analysis-execution）の SQ4 検証完了前にベース版を作成。
- SQ1〜SQ3 は実コンテンツ、SQ4 は検証方法を記述したプレースホルダ（実測結果は後日追記）。
- 公開先 Zenn 記事は `published: false` で作成する。

### Pre-publish final check
- [x] イシュー要旨のメインメッセージと最終出力のメインメッセージが一致
- [x] ストーリーラインが最終出力に保持されている
- [x] 検証根拠が読者に追える（SQ4 は「検証中」と明記）
- [x] 機密情報の漏洩なし

---

## Phase 5b: Output Publish (✅ Done 2026-05-18)

### Generated outputs
- [x] zenn: `./.cadenza/output-zenn.md`（published: false）
- [ ] deck: 未生成
- [ ] lt: 未生成

### Voice profile in use
`./.cadenza/voice-style-notes.md`（過去 Zenn 記事4本から抽出：cadenza / personal-ai-agent / agentcore-optimization / second-me）

### Zenn 化での主な適応
- 文体を「私」一人称・です・ます調・「はじめに／おわりに」構成へ
- 二層ループ図は Mermaid を使わず画像プレースホルダに（著者は過去記事で Mermaid 不使用・画像派）。Mermaid ソースは `materials/concept-batch.md` に保持、publish 時に画像化
- コードブロックを `language:filename` 規約に、検証中の注記を `:::message` に
